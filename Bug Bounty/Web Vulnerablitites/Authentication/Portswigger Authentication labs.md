...
3) by using the happy flow of 2fa login we found "4 digit" otp format is used, then we found no rate limiting on otp, so we did bruteforce on other users otp by changing the name of user in request(Cookie: verify=carlos;) and removing s
```
